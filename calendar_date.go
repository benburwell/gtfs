...
// A CalendarDate is an exception for the service IDs defined in the
// calendar.txt file. If calendar.txt includes ALL dates of service, this file
...
// defined in the calendar.txt file. If your service is generally regular, with
// a few changes on explicit dates (for example, to accommodate special event
...
type CalendarDate struct {
	...
	// a service exception is available for one or more routes. Each (service_id,
	...
	// information in calendar_dates.txt modifies the service information
	...
	// whether service is available on the specified date. The date field’s value
	...
	//
	//     - A value of 1 indicates that service has been added for the specified
	//       date.
	...
	// and another set of trips available on all other days. You could have one
	...
	// holiday, you would use the calendar_dates.txt file to add the holiday to
	// the holiday service_id and to remove the holiday from the regular
	// service_id schedule.
	ExceptionType *ExceptionType `csv:"exception_type"`
}
