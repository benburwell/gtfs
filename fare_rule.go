...
// A FareRule s a rule for applying fare information for a transit
...
type FareRule struct {
	...
	// IDs are referenced from the stops.txt file. If you have several origin IDs
	// with the same fare attributes, create a row in fare_rules.txt for each
	// origin ID.

	// For example, if fare class “b” is valid for all travel originating from
	// either zone “2” or zone “8”, the fare_rules.txt file would contain these
	...
	// The destination_id field associates the fare ID with a destination zone
	// ID. Zone IDs are referenced from the stops.txt file. If you have several
	// destination IDs with the same fare attributes, create a row in
	// fare_rules.txt for each destination ID.
	//
	// For example, you could use the origin_ID and destination_ID fields
	// together to specify that fare class “b” is valid for travel between zones
	// 3 and 4, and for travel between zones 3 and 5, the fare_rules.txt file
	...
	// that pass through every contains_id zone.
	//
	// For example, if fare class “c” is associated with all travel on the GRT
	// route that passes through zones 5, 6, and 7 the fare_rules.txt would
	// contain these rows:
	//
	// c,GRT,,,5
	// c,GRT,,,6
	...
	// fare class “c”. For more detail, see FareExamples in the
