...
// FeedInfo is additional information about the feed itself, including
// publisher, version, and expiration information.
//
// The file contains information about the feed itself, rather than the
// services that the feed describes. GTFS currently has an agency.txt file to
// provide information about the agencies that operate the services described
// by the feed. However, the publisher of the feed is sometimes a different
// entity than any of the agencies (in the case of regional aggregators). In
// addition, there are some fields that are really feed-wide settings, rather
// than agency-wide.
type FeedInfo struct {
	...
	// values in agency.txt.) GTFS-consuming applications can display this name
	// when giving attribution for a particular feed’s data.
	...
	// values in agency.txt.) The value must be a fully qualified URL that
	// includes http:// or https://, and any special characters in the URL must
	// be correctly escaped. See
	...
	// for calendar.txt, or left empty if unavailable. The feed_end_date date
	// must not precede the feed_start_date date if both are given. Feed
	// providers are encouraged to give schedule data outside this period to
	// advise of likely future service, but feed consumers should treat it
	// mindful of its non-authoritative status. If feed_start_date or
	// feed_end_date extend beyond the active calendar dates defined in
	// calendar.txt and calendar_dates.txt, the feed is making an explicit
	// assertion that there is no service for dates within the feed_start_date or
	...
	// value to help feed publishers determine whether the latest version of
	// their feed has been incorporated.
